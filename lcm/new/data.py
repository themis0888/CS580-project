...
class KPCNDataset(torch.utils.data.Dataset):
    def __init__(self, args, train=True):
        ...
        if train:
            ...
            self.names += glob.glob(os.path.join(args.dir_test, "*.pt"))
    ...
    def __getitem__(self, idx):
        ...
        if self.train:
            ...
            # elif self.ext == 'pt':
            elif '.pt' in self.names[idx]:
                img_num = self.names[idx][len(args.dir_test):-3]
                return [img_num, torch.load(self.names[idx])]
            else:
                raise ValueError('No exr or pt file, This message must not be printed!!')
